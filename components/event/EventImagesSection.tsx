...
import { useUploadEventImage, useDeleteEventImage, useSetEventPrimaryImage } from '@/lib/api/mutations'
...
export function EventImagesSection({
  ...
  const setPrimaryImage = useSetEventPrimaryImage()
  ...
  const handleImagePress = (image: EventImage) => {
  ...
  const handleSetPrimary = () => {
    ...
    setPrimaryImage.mutate(
      { imageId: selectedImage.id, eventId },
      {
        onSuccess: () => {
          Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success)
  ...
  const isPrimary = (image: EventImage) => {
    return image.url === primaryImageUrl || image.isPrimary
  }
    ...
    <View style={styles.container}>
      ...
      {images.length === 0 && !isUploading ? (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtext}>Add photos to showcase your event</Text>
        </View>
      ) : (
        <ScrollView
          horizontal
